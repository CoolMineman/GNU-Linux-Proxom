...
JNIEXPORT void JNICALL Java_com_tudor_android_proxom_ProxyThread_runProxy
  (JNIEnv *pEnv, jobject thisObject, jstring javaServerAddress){
    ...
    port  = portAmongUs;
    lport = portAmongUs;
    dhost = create_peer_array(destinationHost, port);
    ...
    if(samesock) {
        if(!quiet) fprintf(fp, "- same socket/port mode\n");
        samesock = bind_udp_socket(NULL, Lhost, 0);
    }
    if(multisock) {
        if(!quiet) fprintf(fp, "- multi socket/port mode\n");
    }
    ...
    if(acpfile) {
        if(!quiet) fprintf(fp, "- create ACP file %s\n", acpfile);
        fdcap = fopen(acpfile, "rb");
        if(fdcap) {
            fclose(fdcap);
            fprintf(fp, "- do you want to overwrite (Y) or append (A) the file? (y/a/N)\n  ");
    ...
    if(!dhost[0].sin_addr.s_addr) {
        ...
        fprintf(fp, "- wait first packet from the server (double-binding mode)\n");
    ...
    for(;running;) {
        ...
        if(sdi && (FD_ISSET(sdi, &readset))) {
            ...
            if(!quiet) fprintf(fp, "- packet injection from %s:%hu (%d bytes)\n",
    ...
    (*pEnv)->ReleaseStringUTFChars(pEnv, javaServerAddress, destinationHost);
...
struct clients_struct *check_sd(struct sockaddr_in *peer, int force_remove) {
    ...
    for(c = clients; c; ) {
        ...
        if((curr - c->timez) >= timeout) {
            if(!quiet) fprintf(fp, "- remove %s:%hu\n",
...
void show_peer_array(u8 *str, struct sockaddr_in *peer) {
    ...
    for(i = 0; peer[i].sin_addr.s_addr; i++) {
        if(i) fprintf(fp, ", ");
        fprintf(fp, "%s:%hu", inet_ntoa(peer[i].sin_addr), ntohs(peer[i].sin_port));
    }
    fputc('\n', stderr);
}
...
int bind_udp_socket(struct sockaddr_in *peer, in_addr_t iface, u16 port) {
    ...
    if((iface != INADDR_ANY) || port) {
        if(!quiet) fprintf(fp, "- bind UDP port %hu on interface %s\n",
